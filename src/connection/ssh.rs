...
impl SshConnection {
    pub fn check_connection(ip: &str, user: Option<&str>) -> Result<(), (Option<i32>, String)> {
        ...
        let output = Command::new("ssh")
            .args(&["-o", "BatchMode=yes", "-o", "ConnectTimeout=3", "-o", "StrictHostKeyChecking=no", &target, "echo 'ok'"])
            .output()
    ...
    pub fn diagnose(status_code: Option<i32>, stderr: &str) -> DiagnosticResult {
        ...
        if stderr.contains("Permission denied") {
             return DiagnosticResult {
                ...
                recommendation: "Check '~/.ssh/authorized_keys' on target or verify Username.".to_string(),
            };
        }
        if stderr.contains("timed out") || stderr.contains("No route to host") || status_code == Some(255) {
             return DiagnosticResult {
                message: "🔌 Network Timeout / Unreachable".to_string(),
                recommendation: "Verify VM is running ('virsh list') and Network Bridge is active.".to_string(),
            };
        }
        if stderr.contains("Host key verification failed") {
             return DiagnosticResult {
                message: "🛡️ Host Key Change Detected (MITM/IP Rotation)".to_string(),
                recommendation: "'ssh-keygen -f \"$HOME/.ssh/known_hosts\" -R <ip_address>'".to_string(),
            };
        }

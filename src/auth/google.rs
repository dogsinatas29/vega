use oauth2::{
    basic::{BasicClient, BasicTokenResponse}, AuthUrl, ClientId, ClientSecret, CsrfToken, RedirectUrl, Scope,
...
pub async fn login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !client_id.ends_with(".apps.googleusercontent.com") {
        return Err("Invalid Client ID format. It should end with '.apps.googleusercontent.com'".into());
    }
    ...
    // Try to open browser
    if let Err(_) = std::process::Command::new("xdg-open").arg(authorize_url.as_str()).spawn() {
    ...
    let url = Url::parse(&format!("http://localhost{}", redirect_url_path))?;
    ...
    let message = if let Some((_, code)) = code_pair {
        ...
        match token_result {
            Ok(token) => {
                ...
                // Save to Keyring
                match crate::security::keyring::set_token("google_oauth_token", secret) {
                    Ok(_) => {
                        println!("✅ Login successful! Token saved.");
                        "Authentication successful! Token saved. You can close this window."
                    },
                    Err(e) => {
                        eprintln!("❌ Failed to save token: {}", e);
                        "Authentication successful, but failed to save token."
                    }
                }
            }
            Err(e) => {
                eprintln!("❌ Token exchange failed: {}", e);
                eprintln!("❌ Login FAILED. Token was NOT saved.");

...
async fn main() {
    ...
    if input == "connect" && args.len() >= 3 {
        let target_name = &args[2];
        println!(
            "🤖 Tiki-Taka: Analyzing request to connect to '{}'...",
            target_name
        );
        ...
        if let Some(entry) = kb.get(target_name).cloned() {
            kb_hit = true;
            println!(
                "📚 Knowledge: Found verified config for '{}' ({})",
                target_name, entry.ip
            );
            ...
            if SshConnection::check_connection(&entry.ip, entry.user.as_deref()).is_ok() {
                ...
                println!(
                    "🔄 Self-Healing: Initiating Rescan for '{}'...",
                    target_name
                );
        ...
        if let Some(vm) = target_vm {
            ...
            println!(
                "❌ Scanning complete. No VM found matching '{}'.",
                target_name
            );
        }
        return;

...
- **인프라 인지 (v0.0.17.15):** 물리적 자원(CPU/RAM/Disk)의 심층 감지 및 도구별 요구사항에 기반한 선제적 유효성 검사 구현.
...
## 🧠 핵심 아키텍처 (Hybrid Pipeline v0.0.17.15)
...
8.  **실행 및 RAW 가시성 (v0.0.17.15)**: 로컬/원격(SSH) 환경에 명령을 집행하며, **직접 스트림 캡처**(STDOUT/STDERR/EXIT_CODE)를 통해 투명성을 확보합니다.
